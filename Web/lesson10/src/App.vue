...
export default {
    ...
    beforeCreate() {
        ...
        this.$root.$on("onRegister", (login, name) => {
            const lowerCaseLatin = new RegExp("^[a-z]+$");
            if (login.length < 4 || login.length > 16) {
                this.$root.$emit("onRegisterValidationError", "Login size must be between 4 and 16");
                return;
            }
            if (!lowerCaseLatin.test(login)) {
                this.$root.$emit("onRegisterValidationError", "Login should contain only lowercase latin letters");
                return;
            }
            const users = Object.values(this.users).filter(u => u.login === login);
            if (users.length !== 0) {
                this.$root.$emit("onRegisterValidationError", "Login in use");
                return;
            }
